...
y1 <- data.climate_time$winter_snow_evaporation_mean
err1 <- data.climate_time$winter_snow_evaporation_stdDev
mdl1 <- lm(y1 ~ x1)

# Summer
x2 <- data.climate_time$year
y2 <- data.climate_time$summer_snow_evaporation_mean
err2 <- data.climate_time$summer_snow_evaporation_stdDev
mdl2 <- lm(y2 ~ x2)

# Autumn
x3 <- data.climate_time$year
y3 <- data.climate_time$autumn_snow_evaporation_mean
err3 <- data.climate_time$autumn_snow_evaporation_stdDev
mdl3 <- lm(y3 ~ x3)

# Spring
x4 <- data.climate_time$year
y4 <- data.climate_time$spring_snow_evaporation_mean
err4 <- data.climate_time$spring_snow_evaporation_stdDev
mdl4 <- lm(y4 ~ x4)
...
# Open PNG ####
png(filename = paste0("~/snowlines/outputs/climate time/plots/climate time plot sublimation.png"),
...
plot(x = x1,
     y = y1,
     xlab = "Time (Years)",
     ylab = "Snow Evaporation/Condensation (SWE)",
     xlim = c(1980, 2022),
     ylim = c(-0.000085, 0.00003),
     yaxs = "i",
     xaxs = "i",
     type = "n",
     frame.plot = FALSE,
...
function.plot_seasons <- function(data_x, data_y, err, mdl, point_shape, colour_adjustment) {
  ...
  arrows(x0 = data_x,
         ...
         angle = 90,
         code = 3,
         col = function.col_adjust(aes.colour$solid_purple, colour_adjustment),
         lty = "solid",
...
axis(side = 2, at = seq(-0.000085, 0.00003, by = 0.00002), lwd = aes.canvas$line_width)
axis(side = 2, at = seq(-0.000085, 0.00003, by = 0.000005), tick = TRUE, labels = FALSE, lwd = aes.canvas$tick_mark_minor_width, tcl = aes.canvas$tick_mark_minor_height)
...
browseURL(url = "~/snowlines/outputs/climate time/plots/climate time plot sublimation.png")
