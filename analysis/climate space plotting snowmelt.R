...
# Winter
x1 <- data.climate$`2017`$distance
y1 <- data.climate$`2017`$winter_snowmelt_mean
err1 <- data.climate$`2017`$winter_snowmelt_stdDev
x1quad <- x1^2
x1cub <- x1^3
x1four <- x1^4
x1five <- x1^5
mdl1 <- lm(y1 ~ x1 + x1quad + x1cub + x1four)
print(anova(mdl1))
plot(y1, summary(mdl1)$residuals)

# Summer
x2 <- data.climate$`2017`$distance
y2 <- data.climate$`2017`$summer_snowmelt_mean
err2 <- data.climate$`2017`$summer_snowmelt_stdDev
x2quad <- x2^2
x2cub <- x2^3
mdl2 <- lm(y2 ~ x2 + x2quad)
#print(anova(mdl2))
#plot(y2, summary(mdl2)$residuals)

# Autumn
x3 <- data.climate$`2017`$distance
y3 <- data.climate$`2017`$autumn_snowmelt_mean
err3 <- data.climate$`2017`$autumn_snowmelt_stdDev
...
# Spring
x4 <- data.climate$`2017`$distance
y4 <- data.climate$`2017`$spring_snowmelt_mean
err4 <- data.climate$`2017`$spring_snowmelt_stdDev
x4quad <- x4^2
x4cub <- x4^3
x4four <- x4^4
x4five <- x4^5
mdl4 <- lm(y4 ~ x4 + x4quad + x4cub + x4four)
print(anova(mdl4))
plot(y4, summary(mdl4)$residuals)

# Open SVG ####
svg(filename = paste0("~/snowlines/outputs/climate space/plots/climate space plot snowmelt.svg"),
...
function.plot_seasons(x1, y1, err1, mdl1, aes.symbols$datapoint_shape, 0.1)
function.plot_seasons(x2, y2, err2, mdl2, aes.symbols$datapoint_shape, 1.0)
function.plot_seasons(x3, y3, err3, mdl3, aes.symbols$datapoint_shape, 0.7)
function.plot_seasons(x4, y4, err4, mdl4, aes.symbols$datapoint_shape, 0.4)
...
axis(side = 2, at = seq(0, 0.008, by = 0.001), lwd = aes.canvas$line_width)
axis(side = 2, at = seq(0, 0.008, by = 0.0005), tick = TRUE, labels = FALSE, lwd = aes.canvas$tick_mark_minor_width, tcl = aes.canvas$tick_mark_minor_height)
...
browseURL(url = "~/snowlines/outputs/climate space/plots/climate space plot snowmelt.svg")
