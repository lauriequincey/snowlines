...
# Reclassify ####
# Winter
x1 <- data.snowlines_climate$`2017`$winter_surface_solar_radiation_downwards_mean/1000000
...
# Summer
x2 <- data.snowlines_climate$`2017`$summer_surface_solar_radiation_downwards_mean/1000000
...
# Autumn
x3 <- data.snowlines_climate$`2017`$autumn_surface_solar_radiation_downwards_mean/1000000
...
# Spring
x4 <- data.snowlines_climate$`2017`$spring_surface_solar_radiation_downwards_mean/1000000
...
# Open SVG ####
svg(filename = paste0("~/snowlines/outputs/snowline climate/plots/snowline climate plot radiation.svg"),
...
plot(x = x1,
     y = y1,
     xlab = "Solar Downwelling Radiaiton (Mj m-1)",
     ylab = "Altitude (m asl)",
     xlim = c(0.5, 9),#c(6.3, 9),
     ylim = c(0, 2500),
     yaxs = "i",
     xaxs = "i",
     type = "n",
...
function.plot_seasons(x2, y2, err2yplus, err2yminus, mdl2, aes.symbols$datapoint_shape, 1.0)
function.plot_seasons(x3, y3, err3yplus, err3yminus, mdl3, aes.symbols$datapoint_shape, 0.8)
...
axis(side = 2, at = seq(0, 2500, by = 500), lwd = aes.canvas$line_width)
...
axis(side = 1, at = seq(0, 9, 1), lwd = aes.canvas$line_width)
axis(side = 1, at = seq(0, 9.5, 0.5), tick = TRUE, labels = FALSE, lwd = aes.canvas$tick_mark_minor_width, tcl = aes.canvas$tick_mark_minor_height)

# Close SVG ####
dev.off()

browseURL(url = "~/snowlines/outputs/snowline climate/plots/snowline climate plot radiation.svg")