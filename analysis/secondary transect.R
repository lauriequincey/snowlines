# Import ####
data.snowlines <- read.csv("~/snowlines/upload/canada/kitkatla2_LANDSAT8_DAT20210701-20210717.csv")

# Prep ####

# Find the middle distance of all the years distance ranges
middle_distance <- median(data.snowlines$distance)
...
# Generate rsla ####
x <- data.snowlines
data.snowlines_rsla <- function.rsla_generation(x, x$altitude, x$distance, x$unix_aqcuisition_time, x$aspect, x$altitudinal_translation_error, x$elevation_base_error)

# Reclassify ####
x <- data.snowlines$distance
y <- data.snowlines$altitude
...
# Open SVG ####
svg(filename = paste0("~/snowlines/outputs/snowline space/plots/snowline space plot sonora.svg"),
...
axis(side = 2,
     at = c(0, 500, 1000, 1500, 2000, 2500, 3000, 3500),
     lwd = aes.canvas$line_width)
axis(side = 2, at = seq(0, 3500, by = 100), tick = TRUE, labels = FALSE, lwd = aes.canvas$tick_mark_minor_width, tcl = aes.canvas$tick_mark_minor_height)
...
browseURL(url = "~/snowlines/outputs/snowline space/plots/snowline space plot sonora.svg")
