...
# Reclassify ####
x <- data.validation$terra_snow_cover_percentage
y <- data.validation$landsat_snow_cover_percentage

# Open SVG ####
svg(filename = paste0("~/snowlines/outputs/validation/validation plot.svg"),
...
plot(x = x,
     y = y,
     xlab = "MOD10A1 Snow Extent (%)",
     ylab = "Study Algorithm Snow Extent (%)",
     xlim = c(0, 80),
     ylim = c(0, 80),
...
browseURL("~/snowlines/outputs/validation/validation plot.svg")
