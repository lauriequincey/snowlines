...
y1 <- data.climate_time$winter_temperature_2m_mean_converted
err1 <- data.climate_time$winter_temperature_2m_stdDev
mdl1 <- lm(y1 ~ x1)

# Summer
x2 <- data.climate_time$year
y2 <- data.climate_time$summer_temperature_2m_mean_converted
err2 <- data.climate_time$summer_temperature_2m_stdDev
mdl2 <- lm(y2 ~ x2)

# Autumn
x3 <- data.climate_time$year
y3 <- data.climate_time$autumn_temperature_2m_mean_converted
err3 <- data.climate_time$autumn_temperature_2m_stdDev
mdl3 <- lm(y3 ~ x3)

# Spring
x4 <- data.climate_time$year
y4 <- data.climate_time$spring_temperature_2m_mean_converted
err4 <- data.climate_time$spring_temperature_2m_stdDev
mdl4 <- lm(y4 ~ x4)

# Open SVG ####
svg(filename = paste0("~/snowlines/outputs/climate time/plots/climate time plot temperature.svg"),
...
function.plot_seasons <- function(data_x, data_y, err, mdl, point_shape, colour_adjustment) {
  clip(x1 = -999999999999999, x2 = 999999999999999,
       y1 = -999999999999999, y2 = 999999999999999)
...
function.plot_seasons(x1, y1, err1, mdl1, aes.symbols$datapoint_shape, 0.6)
...
function.plot_seasons(x4, y4, err4, mdl4, aes.symbols$datapoint_shape, 0.8)
...
axis(side = 2, at = seq(-15, 28, by = 10), lwd = aes.canvas$line_width)
axis(side = 2, at = seq(-15, 28, by = 1), tick = TRUE, labels = FALSE, lwd = aes.canvas$tick_mark_minor_width, tcl = aes.canvas$tick_mark_minor_height)
...
browseURL(url = "~/snowlines/outputs/climate time/plots/climate time plot temperature.svg")
