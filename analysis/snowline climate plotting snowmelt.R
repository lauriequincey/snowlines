...
# Reclassify ####
# Winter
x1 <- data.snowlines_climate$`2017`$winter_snowmelt_mean
y1 <- data.snowlines_climate$`2017`$altitude
#err1x <- data.snowlines_climate$`2017`$winter_temperature_2m_stdDev
err1yplus <- data.snowlines_rsla$plus_error[30]
err1yminus <- data.snowlines_rsla$minus_error[30]
x1quad <- x1^2
x1cub <- x1^3
x1four <- x1^4
x1five <- x1^5
...
# Summer
x2 <- data.snowlines_climate$`2017`$summer_snowmelt_mean
y2 <- data.snowlines_climate$`2017`$altitude
#err2x <- data.snowlines_climate$`2017`$summer_temperature_2m_stdDev
err2yplus <- data.snowlines_rsla$plus_error[30]
err2yminus <- data.snowlines_rsla$minus_error[30]
x2quad <- x2^2
x2cub <- x2^3
x2four <- x2^4
x2five <- x2^5
...
# Autumn
x3 <- data.snowlines_climate$`2017`$autumn_snowmelt_mean
y3 <- data.snowlines_climate$`2017`$altitude
#err3x <- data.snowlines_climate$`2017`$autumn_temperature_2m_stdDev
err3yplus <- data.snowlines_rsla$plus_error[30]
err3yminus <- data.snowlines_rsla$minus_error[30]
x3quad <- x3^2
x3cub <- x3^3
x3four <- x3^4
x3five <- x3^5
...
# Spring
x4 <- data.snowlines_climate$`2017`$spring_snowmelt_mean
y4 <- data.snowlines_climate$`2017`$altitude
#err1x <- data.snowlines_climate$`2017`$winter_temperature_2m_stdDev
err4yplus <- data.snowlines_rsla$plus_error[30]
err4yminus <- data.snowlines_rsla$minus_error[30]
x4quad <- x4^2
x4cub <- x4^3
x4four <- x4^4
x4five <- x4^5
...
# Open SVG ####
svg(filename = paste0("~/snowlines/outputs/snowline climate/plots/snowline climate plot snowmelt.svg"),
...
function.plot_seasons(x1, y1, err1yplus, err1yminus, mdl1, aes.symbols$datapoint_shape, 0.3)
function.plot_seasons(x2, y2, err2yplus, err2yminus, mdl2, aes.symbols$datapoint_shape, 1.2)
function.plot_seasons(x3, y3, err3yplus, err3yminus, mdl3, aes.symbols$datapoint_shape, 0.9)
function.plot_seasons(x4, y4, err4yplus, err4yminus, mdl4, aes.symbols$datapoint_shape, 0.6)
...
axis(side = 1, at = seq(0, 0.004, 0.001), lwd = aes.canvas$line_width)
axis(side = 1, at = seq(0, 0.004, 0.0005), tick = TRUE, labels = FALSE, lwd = aes.canvas$tick_mark_minor_width, tcl = aes.canvas$tick_mark_minor_height)

# Close SVG ####
dev.off()

browseURL(url = "~/snowlines/outputs/snowline climate/plots/snowline climate plot snowmelt.svg")