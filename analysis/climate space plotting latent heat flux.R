...
y1 <- data.climate$`2018`$winter_surface_latent_heat_flux_mean
err1 <- data.climate$`2018`$winter_surface_latent_heat_flux_stdDev
mdl1 <- lm(y1 ~ poly(x1, 3))

# Summer
x2 <- data.climate$`2018`$distance
y2 <- data.climate$`2018`$summer_surface_latent_heat_flux_mean
err2 <- data.climate$`2018`$summer_surface_latent_heat_flux_stdDev
mdl2 <- lm(y2 ~ poly(x2, 3))

# Autumn
x3 <- data.climate$`2018`$distance
y3 <- data.climate$`2018`$autumn_surface_latent_heat_flux_mean
err3 <- data.climate$`2018`$autumn_surface_latent_heat_flux_stdDev
mdl3 <- lm(y3 ~ poly(x3, 3))

# Spring
x4 <- data.climate$`2018`$distance
y4 <- data.climate$`2018`$spring_surface_latent_heat_flux_mean
err4 <- data.climate$`2018`$spring_surface_latent_heat_flux_stdDev
...
# Open PNG ####
png(filename = paste0("~/snowlines/outputs/climate space/plots/climate space plot latent heat flux.png"),
...
plot(x = x1,
     y = y2,
     xlab = "Distance (km)",
     ylab = "Surface Latent Heat Flux (Jm-2)",
     xlim = c(0, 250),
...
browseURL(url = "~/snowlines/outputs/climate space/plots/climate space plot latent heat flux.png")
